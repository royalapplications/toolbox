...
.NOTES
  ...
  Copyright:      (C) 2018 code4ward.net GmbH
  Creation Date:  April 25, 2018
  Modified Date:  April 25, 2018
...
if (!(Get-Module "RoyalDocument.PowerShell")) {
    Write-Host "RoyalDocument module not loaded."
    Write-Host "Be sure Royal TS is installed and check if RoyalPowerShellModule is set."
    Write-Host "Aborting."
    exit
}
...
# sanity checks
if (!(Test-Path $File)) {
    Write-Host "Royal Document '$File' does not exist. Please provide a existing file. Aborting."
    exit
}
$RoyalDocFile = $File
...
# open document
Write-Host -Verbose "+ Loading document..."
...
if ($doc -eq $null) {
    Write-Host "Failed loading document. Please check. Aborting."
    exit
}

Write-Host -Verbose "+ Loading credentials..."
...
Write-Host -Verbose "Total of unique passwords to check: $totalPasswords"
Write-Host -Verbose "Total of objects with passwords found: $totalObjects"

# now we're checking the passwords...
Write-Host -Verbose "+ Checking passwords against HIBP..."
$totalMatches = 0
$hashedPwdCount = 0
$docHashedPasswords.Keys | ForEach-Object {
    ...
    if ($hashMatches -isnot [int]) {
        Write-Host " Error ocurred while checking: $hashMatches"
        continue
    }
...
# ...and we're done.
Write-Host -Verbose "+ Password check done."
